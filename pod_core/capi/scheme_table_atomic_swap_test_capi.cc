...
class WrapperAlice {
 public:
  WrapperAlice(handle_t c_alice_data, uint8_t const* c_self_id,
    ...
    h_ = E_TableAtomicSwapAliceNew(c_alice_data, c_self_id, c_peer_id);
    if (!h_) throw std::runtime_error("");
  }
  ~WrapperAlice() {
    if (!E_TableAtomicSwapAliceFree(h_)) abort();
  }
  handle_t h() const { return h_; }
...
class WrapperBob {
 public:
  WrapperBob(handle_t c_bob_data, uint8_t const* c_self_id,
    ...
    h_ = E_TableAtomicSwapBobNew(c_bob_data, c_self_id, c_peer_id, c_demand,
                                 c_demand_count);
    if (!h_) throw std::runtime_error("");
  }
  ~WrapperBob() {
    if (!E_TableAtomicSwapBobFree(h_)) abort();
  }
  handle_t h() const { return h_; }
...
bool Test(std::string const& output_path, WrapperAliceData const& a,
  ...
  if (evil) E_TableAtomicSwapAliceSetEvil(alice.h());
  ...
  if (!E_TableAtomicSwapBobGetRequest(bob.h(), request_file.c_str())) {
    assert(false);
    return false;
  }

  if (!E_TableAtomicSwapAliceOnRequest(alice.h(), request_file.c_str(),
                                       response_file.c_str())) {
    assert(false);
    return false;
  }

  if (!E_TableAtomicSwapBobOnResponse(bob.h(), response_file.c_str(),
                                      receipt_file.c_str())) {
    assert(false);
    return false;
  }

  if (!E_TableAtomicSwapAliceOnReceipt(alice.h(), receipt_file.c_str(),
                                       secret_file.c_str())) {
    assert(false);
    return false;
  }
  ...
  if (!evil) {
    if (!E_TableAtomicSwapBobOnSecret(bob.h(), secret_file.c_str())) {
      assert(false);
      return false;
    }

    if (!E_TableAtomicSwapBobSaveDecrypted(bob.h(), output_file.c_str())) {
      assert(false);
      return false;
    }
  } else {
