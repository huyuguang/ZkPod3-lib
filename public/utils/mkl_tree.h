...
inline static const h256_t kEmptyH256 = h256_t();

inline void TwoToOne(h256_t const& a, h256_t const& b, h256_t* r) {
...
// return mkl root
inline h256_t CalcPath(GetItem get_item, uint64_t item_count, uint64_t leaf,
                       Path* path) {
...
inline h256_t CalcRoot(GetItem get_item, uint64_t item_count) {
...
inline bool CalcRoot(std::string const& file, h256_t* root) {
  try {
...
inline std::vector<Range> SplitRange(Range const& range) {
...
inline bool VerifyRangePath(GetItem get_item, Range const& range,
                            uint64_t item_count, h256_t const& root,
                            Path const& path) {
...
inline size_t GetTreeSize(uint64_t item_count) {
  if (item_count == 1) return 1;
  return misc::Pow2UB(item_count) - 1;
}

inline Tree BuildTree(uint64_t item_count, GetItem const& get_item) {
