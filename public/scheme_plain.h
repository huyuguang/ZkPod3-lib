...
inline uint64_t GetDataBlockCount(uint64_t size, uint64_t column_num) {
  uint64_t slice_count = (size + 31 - 1) / 31;
  return (slice_count + column_num - 1) / column_num;
}

inline bool DataToM(std::string const& pathname, uint64_t size, uint64_t n,
...
inline bool DecryptedRangeMToFile(std::string const& file, uint64_t size,
                                  uint64_t s, uint64_t start, uint64_t count,
