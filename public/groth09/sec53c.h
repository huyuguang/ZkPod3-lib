...
void serialize(Ar& ar, CommitmentExtPub const& t) {
  ar& YAS_OBJECT_NVP("53c.cep", ("cl", t.cl), ("cu", t.cu));
}
...
struct Proof {
  ...
  sec51c::Proof proof_51;        // 4 G1, 2n+3 Fr
...
void serialize(Ar& ar, Proof const& t) {
  ar& YAS_OBJECT_NVP("53c.pf", ("c", t.com_ext_pub), ("51", t.proof_51));
}
...
inline void ProveFinal(Proof& proof, h256_t const& seed,
                       ProverInput const& input, CommitmentPub const& com_pub,
                       CommitmentSec const& com_sec) {
  // Tick tick(__FUNCTION__);
  assert(input.m() == 1);

  sec51c::ProverInput input_51(input.x[0], input.y[0], input.t, input.yt[0],
                               input.z, input.x_g_offset, input.y_g_offset,
                               input.z_g_offset);

  sec51c::CommitmentPub com_pub_51(com_pub.a[0], com_pub.b[0], com_pub.c);
  sec51c::CommitmentSec com_sec_51(com_sec.r[0], com_sec.s[0], com_sec.t);
  sec51c::Prove(proof.proof_51, seed, input_51, com_pub_51, com_sec_51);
}
...
inline void ProveRecursive(Proof& proof, h256_t& seed, ProverInput& input,
...
inline bool Verify(Proof const& proof, h256_t seed,
  ...
  sec51c::CommitmentPub com_pub_51(com_pub.a[0], com_pub.b[0], com_pub.c);
  sec51c::VerifierInput verifier_input_51(input.t, com_pub_51, input.x_g_offset,
                                          input.y_g_offset, input.z_g_offset);
  return sec51c::Verify(proof.proof_51, seed, verifier_input_51);
}
