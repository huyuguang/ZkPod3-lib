...
struct VrsBasic {
  ...
  // since cache always use 0 as g_offset
  static int64_t const kR1csGOffset = 0;

  struct ProveInput {
    ProveInput(int64_t n, Fr const& k, Fr const& k_com_r, GetP&& iget_p,
               GetW&& iget_w, Fr const& vw_com_r, int64_t vw_g_offset)
        : k(k),
          k_com_r(k_com_r),
    ...
    void BuildVarComs(std::vector<std::vector<Fr>> const& vars,
      ...
      if ((int64_t)icached_var_coms.size() == m) {
        ...
        auto parallel_f = [this, &vars](int64_t i) {
          var_coms[i] =
              PcComputeCommitmentG(kR1csGOffset, vars[i], var_coms_r[i]);
        };
  ...
  static Fr Prove(Proof& proof, ProveOutput& output, h256_t seed,
    ...
    typename HyraxA::ProveInput ip_input(v, w, vw, kR1csGOffset,
                                         input.vw_g_offset);
  ...
  struct VerifyInput {
    VerifyInput(int64_t n, GetP&& iget_p, GetW&& iget_w, int64_t vw_g_offset)
        : get_p(std::move(iget_p)),
          get_w(std::move(iget_w)),
          vw_g_offset(vw_g_offset),
          scheme(new Scheme),
  ...
  static bool Verify(VerifyOutput& output, Proof const& proof, h256_t seed,
    ...
    // check com_plain
    if (proof.var_coms[0] != PcComputeCommitmentG(kR1csGOffset, p, FrZero())) {
      assert(false);
      return false;
    }
    ...
    typename HyraxA::VerifyInput ip_input(w, ip_com_pub, kR1csGOffset,
                                          input.vw_g_offset);
