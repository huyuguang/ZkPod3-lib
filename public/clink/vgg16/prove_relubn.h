...
inline void ReluBnInOutProvePreprocess(h256_t seed, ProveContext const& context,
  ...
  adapt_item_in.Init(images.size() / 2, "relubn prove in");
  adapt_item_out.Init(images.size() / 2, "relubn prove out");
...
inline void ReluBnProvePreprocess(h256_t seed, ProveContext const& context,
                                  ...
                                  ParallelVoidTaskMan& task_man) {
  ReluBnInOutSec io_sec;
  ReluBnInOutProvePreprocess(seed, context, proof, io_sec, item_man);

  std::shared_ptr<ReluBnR1csSec> r1cs_sec(new ReluBnR1csSec);
  ReluBnR1csProvePreprocess(seed, context, io_sec, proof, r1cs_sec, task_man);
}
};  // namespace clink::vgg16
