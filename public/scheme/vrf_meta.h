...
struct VrfKeyMeta {
  ...
  bool operator!=(VrfKeyMeta const& v) const { return !((*this) == v); }
};
...
struct VrfMeta {
  ...
  bool operator!=(VrfMeta const& v) const { return !((*this) == v); }
};

inline bool SaveVrfMeta(std::string const& output, VrfMeta const& data) {
  if (!data.valid()) {
    assert(false);
    return false;
  }
...
inline bool LoadVrfMeta(std::string const& input, h256_t const* digest,
                        VrfMeta& data) {
  if (digest) {
    h256_t check_digest;
...
inline bool SaveVrfPk(std::string const& output, vrf::Pk<> const& pk) {
...
inline bool LoadVrfPk(std::string const& file, h256_t const* digest,
                      vrf::Pk<>& pk) {
  if (digest) {
    h256_t check_digest;
...
inline bool SaveVrfSk(std::string const& output, vrf::Sk<> const& sk) {
...
inline bool LoadVrfSk(std::string const& file, vrf::Sk<>& sk) {
  const uint64_t kFrBufSize = 32;
  try {
    io::mapped_file_params params;
    params.path = file;
...
inline bool SaveBpP1Proof(std::string const& output, bp::P1Proof const& proof) {
  try {
    io::mapped_file_params params;
    params.path = output;
    params.flags = io::mapped_file_base::readwrite;
...
inline bool LoadBpP1Proof(std::string const& file, h256_t const& digest,
                          bp::P1Proof& proof) {
  h256_t digest2;
...
inline void BuildKeyBp(uint64_t n, uint64_t s, std::vector<Fr> const& m,
...
inline bool VerifyKeyBp(uint64_t n, uint64_t s, std::vector<Fr> const& km,
...
inline bool VerifyKeyBp(std::string const& file, Bulletin const& bulletin,
                        VrfMeta const& vrf_meta, uint64_t j,
                        std::vector<Fr> const& km,
...
inline h256_t HashVrfKey(std::string const& k, vrf::Sk<> const& vrf_sk) {
...
inline bool LoadKeyM(std::string const& input, uint64_t n, bool unique,
