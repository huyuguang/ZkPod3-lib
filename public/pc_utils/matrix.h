...
struct ProverInput {
  ProverInput(GetMatrix get_m, std::vector<G1> const& com_rows,
...
inline void Prove(Proof& proof, h256_t seed, ProverInput const& input) {
  ...
  details::UpdateSeed(seed, input.com_rows, input.com_cols);
  std::vector<Fr> d(n);
  ...
  equal_ip::ProverInput eip_input(v, e, com_v, com_v_r, input.r_g_offset, w, d,
                                  com_w, com_w_r, input.c_g_offset, z);
  equal_ip::Prove(proof, seed, eip_input);
}

struct VerifierInput {
  VerifierInput(std::vector<G1> const& com_rows, int64_t r_g_offset,
...
inline bool Verify(h256_t seed, VerifierInput const& input,
  ...
  equal_ip::VerifierInput eip_input(e, com_v, input.r_g_offset, d, com_w,
                                    input.c_g_offset);
...
inline bool Test(int64_t n, int64_t s) {
  ...
  ProverInput prover_input(std::move(get_m), com_rows, com_row_rs, r_g_offset,
                           com_cols, com_col_rs, c_g_offset);

  Proof proof;
  Prove(proof, seed, prover_input);

  VerifierInput verifier_input(com_rows, r_g_offset, com_cols, c_g_offset);
