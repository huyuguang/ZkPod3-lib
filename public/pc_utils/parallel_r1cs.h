...
void BuildHpCom(int64_t m, int64_t n, std::vector<G1> const& com_w,
                ...
                int64_t g_offset, groth09::sec43b::CommitmentPub& com_pub,
...
void DebugCheckHpCom(int64_t m, groth09::sec43b::ProverInput const& input,
  ...
  for (int64_t i = 0; i < m; ++i) {
    ...
    G1 check_com_pub_a = PcComputeCommitmentG(input.x_g_offset, xi, com_sec_r);
    assert(check_com_pub_a == com_pub_a);

    auto const& yi = input.y[i];
    G1 check_com_pub_b = PcComputeCommitmentG(input.y_g_offset, yi, com_sec_s);
    assert(check_com_pub_b == com_pub_b);

    auto const& zi = input.z[i];
    G1 check_com_pub_c = PcComputeCommitmentG(input.z_g_offset, zi, com_sec_t);
...
struct ProverInput {
  ProverInput(libsnark::protoboard<Fr> const& pb,
              std::vector<std::vector<Fr>>&& iw, std::vector<G1> const& com_w,
    ...
    auto parallel_f = [this](int64_t j) {
...
struct VerifierInput {
  VerifierInput(int64_t n, libsnark::protoboard<Fr> const& pb,
                std::vector<G1> const& com_w,
  ...
  bool Check() const {
    ...
    auto parallel_f = [this](int64_t i) {
      return com_w[i] == PcComputeCommitmentG(g_offset, public_w[i], FrZero());
    };
